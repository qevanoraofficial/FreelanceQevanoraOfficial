import FreelanceAddForm from "@/components/admin/FreelanceAddForm";
import {
    ...
    return (
      <svg width="28" height="28" viewBox="0 0 24 24" fill="none" aria-hidden="true">
        <path
          ...
          strokeLinejoin="round"
        />
        <path d="m14.5 6.7 2.8 2.8" stroke="currentColor" strokeWidth="1.8" />
      </svg>
    );
  }
...
export default async function AdminCrudPage({ params }: Props) {
  ...
  if (!isSection(section) || !isAction(action)) {
    notFound();
  }

  const sectionKey = section as SectionKey;
  const actionKey = action as ActionKey;
  const isFreelanceAdd = sectionKey === "freelance" && actionKey === "add";
    ...
    <main className="min-w-0">
      <section className="relative overflow-hidden rounded-3xl border border-brand-500/15 bg-[#031126] p-5 text-white shadow-[0_20px_70px_rgba(0,0,0,0.24)] sm:p-7">
        ...
        <div className="relative flex items-center gap-4">
          <span className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl border border-brand-500/20 bg-brand-500/10 text-brand-300">
          ...
          <div>
            ...
            <h1 className="mt-1 text-2xl font-black sm:text-3xl">
              {actionLabels[actionKey]} {sectionLabels[sectionKey]}
            </h1>
          </div>
        </div>
      </section>
        ...
        <section className="mt-6 rounded-3xl border border-brand-500/15 bg-[#031126] p-5 text-white sm:p-7">
          <div className="rounded-2xl border border-dashed border-brand-500/20 bg-white/[0.02] px-5 py-12 text-center">
            <p className="text-sm font-bold text-brand-300">
              HALAMAN {actionLabels[actionKey]} {sectionLabels[sectionKey]}
            </p>
            <p className="mx-auto mt-3 max-w-xl text-sm leading-6 text-gray-500">
              Sub halaman sudah aktif dan siap diisi fitur pengelolaan.
            </p>
          </div>
        </section>
      )}
