import ManagedContentManager from "@/components/admin/ManagedContentManager";
import {
...
export default async function AdminCrudPage({ params }: Props) {
  ...
  if (!isManagedSection(section) || !isAction(action)) {
    notFound();
  }
  ...
  try {
    items = await getManagedContentItems(section);
  } catch (cause) {
    error =
        ...
        <div className="relative flex items-center gap-4">
          ...
          <div>
            ...
            <h1 className="mt-1 text-2xl font-black sm:text-3xl">
              {actionLabels[action]} {MANAGED_SECTION_LABELS[section]}
            </h1>
          </div>
        </div>
      </section>

      <ManagedContentManager
        section={section}
        action={action}
        initialItems={items}
